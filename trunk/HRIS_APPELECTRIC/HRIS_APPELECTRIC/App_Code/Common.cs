...
public class Common
{
    ...
    public string FormatTime(DateTime date)
    {
        DateTime dttemp = new DateTime();
        dttemp = date;
        string ret = dttemp.ToString("hh:mm tt");
        return ret;
    }
    ...
    public bool ItemExist(string tbl, string field, string condition)
    {
        return ItemExist(tbl, field, condition, "");
    ...
    public double get_Time_Difference(string starttime, string endTime, int timeOption)
    {
        ...
        try
        {
            TimeSpan tsstarttime = new TimeSpan();
            TimeSpan tsendtime = new TimeSpan();
            if (TimeSpan.TryParse(endTime, out tsendtime) && TimeSpan.TryParse(starttime, out tsstarttime))
            {
    ...
    public void GetThreeDataFromDB(string field1, string field2, string field3, string tbl, string condition, out string val1, out string val2, out string val3)
    {
        val1 = "";
        val2 = "";
        val3 = "";
        ...
        conn_fn.Open();
        SqlDataReader dread_fn;
        SqlCommand cmd1 = new SqlCommand("Select " + field1 + ", " + field2 + ", " + field3 + " from " + tbl + " " + condition + "", conn_fn);
        dread_fn = cmd1.ExecuteReader();
        dread_fn.Read();
        if (dread_fn.HasRows)
        {
            val1 = dread_fn[0].ToString();
            val2 = dread_fn[1].ToString();
            val3 = dread_fn[2].ToString();
        }
        dread_fn.Close();
        conn_fn.Close();
    ...
    public void GetTwoDataFromDB(string field1, string field2, string tbl, string condition, out string val1, out string val2)
    {
        val1 = "";
        val2 = "";
        SqlConnection conn_fn = new SqlConnection(connectionstring);
        conn_fn.Open();
        SqlDataReader dread_fn;
        SqlCommand cmd1 = new SqlCommand("Select " + field1 + ", " + field2 + " from " + tbl + " " + condition + "", conn_fn);
        dread_fn = cmd1.ExecuteReader();
        dread_fn.Read();
        if (dread_fn.HasRows)
        {
    ...
    public bool isHasSchedule(string empno, string bussdate)
    {
        bool ret = false;
        ...
        conn_fn.Open();
        SqlDataReader dread_fn;
        SqlCommand cmd1 = new SqlCommand("Select * from TBL_FLEXISCHED where EmpID = '" + empno + "' AND BussDate = '" + FormatDateyyyy(bussdate) + "'", conn_fn);
        dread_fn = cmd1.ExecuteReader();
        dread_fn.Read();
        if (dread_fn.HasRows)
        {
            if (dread_fn["Sched_TimeIn"].ToString() != "RD" && dread_fn["Sched_TimeOut"].ToString() != "RD")
            {
                ret = true;
            }
        }
    ...
    public string replaceSingleQuote(string stringWithSingleQuote)
    {
        string ret = "";
        string withDoubleQuotes = "";
        if (stringWithSingleQuote.Contains("'"))
            withDoubleQuotes = stringWithSingleQuote.Replace("'", "''");
        else
            withDoubleQuotes = stringWithSingleQuote;

        ret = withDoubleQuotes;
        return ret;
    }
    public List<string> GetListDataFromDB(string field, string tbl, string condition)
    {
    ...
    public int UploadBulkLoanEntries(DataTable dtexcel, out DataTable dtable)
    {
        DataTable tempdb = new DataTable();
        int ret = 0;
        tempdb = dtexcel;
        var AllInRecords = (from row in tempdb.AsEnumerable()
                            select new
                            {
                                ...
                                EmpID = row.Field<string>("EmpID"),
                                DateFiled = row.Field<string>("DateFiled"),
                                DedStart = row.Field<string>("DedStart"),
                                LoanCode = row.Field<string>("LoanCode"),
                                LoanAmount = row.Field<string>("LoanAmount"),
                                DedAmount = row.Field<string>("DedAmount"),
                                AmountPaid = row.Field<string>("AmountPaid"),
        ...
        try
        {
            foreach (var rec in AllInRecords)
            {
                if (rec.EmpID == "" || rec.LoanCode == "")
                {
                    continue;
                }
                if (!ItemExist("TBL_LOANS", "id", "where id = " + rec.LoanCode + "", ""))
                {
                    continue;
                }
                var newString = rec.EmpID.PadLeft(4, '0');
                bool isEmpExist = ItemExist("TBL_EMPLOYEE_MASTER", "Emp_EmpID", "where Emp_EmpID = '" + rec.EmpID.ToString() + "'");
                bool isEmpFormattedExist = ItemExist("TBL_EMPLOYEE_MASTER", "Emp_EmpID", "where Emp_EmpID = '" + newString + "'");
                string getEmpID = isEmpExist ? rec.EmpID.ToString() : isEmpFormattedExist ? newString : rec.EmpID.ToString();
                if (getEmpID == "")
                {
                    continue;
                }
                ...
                recordcount++;
                Dictionary<string, string> param = new Dictionary<string, string>();
                double LoanAmount = 0, AmountPaid = 0, Balance = 0, DedAmount = 0;
                string strLoanName = "";
                strLoanName = GetSpecificDataFromDB("LoanDesc", "TBL_LOANS", "where id = " + rec.LoanCode + "");
                double.TryParse(rec.LoanAmount, out LoanAmount);
                double.TryParse(rec.AmountPaid, out AmountPaid);
                double.TryParse(rec.Balance, out Balance);
                double.TryParse(rec.DedAmount, out DedAmount);
                DateTime DedDate = new DateTime();
                DedDate = dtnow();
                DateTime DateFiled = new DateTime();
                DateFiled = dtnow();
                DateTime.TryParse(rec.DateFiled, out DateFiled);
                param.Add("EmpID", "'" + getEmpID + "'");
                param.Add("DateFiled", "'" + DateFiled + "'");
                param.Add("DedStart", "'" + DedDate.ToString() + "'");
                param.Add("LoanCode", "'" + rec.LoanCode + "'");
                param.Add("Name", "'" + strLoanName + "'");
                param.Add("LoanAmount", "" + LoanAmount + "");
                param.Add("DedAmount", "" + DedAmount + "");
                param.Add("AmountPaid", "" + AmountPaid + "");
                param.Add("Balance", "" + Balance + "");
                param.Add("Loan_Status", rec.Loan_Status == "1" ? "'1'" : "'0'");
                param.Add("DedDate", DedDate.Day.ToString());
                if (ItemExist("TBL_LOANENTRIES", "id", "where EmpID = '" + getEmpID + "' AND LoanCode = '" + rec.LoanCode + "'", ""))
                {
                    if (UpdateQueryCommon(param, "TBL_LOANENTRIES", "EmpID = '" + getEmpID + "' AND LoanCode = '" + rec.LoanCode + "'"))
                    {
                        successupload++;
                        Dictionary<string, string> userparam = new Dictionary<string, string>();
                    }
                ...
                {
                    InsertQueryCommon(param, "TBL_LOANENTRIES");
                }
                //failedupload++;
            }
    ...
    public string RemoveSpecialCharactersForFileName(string input)
    {
        ...
        Regex.Replace(input, @"[^0-9a-zA-Z\s_]+", "");
        //input = input.Replace(" ", "");
        input = input.Replace("(", "");
        input = input.Replace(")", "");
        return input;
